...
data['H1GI4'] = data['H1GI4'].convert_objects(convert_numeric=True) #Are you hispanic or latino
#6. What is your race? You may give more than one answer
data['H1GI6A'] = data['H1GI6A'].convert_objects(convert_numeric=True) #White
data['H1GI6B'] = data['H1GI6B'].convert_objects(convert_numeric=True) #Black or African American
data['H1GI6C'] = data['H1GI6C'].convert_objects(convert_numeric=True) #American Indian or Native American
data['H1GI6D'] = data['H1GI6D'].convert_objects(convert_numeric=True) #Asian or Pacific Islander

#Set missing data to NAN (6 = refused | 8 = don't know)
...
def ETHNICITY (row):
   if row['NUMETHNIC'] > 1 : 
      return 1 #multiple ethnicity
   if row['H1GI4'] == 1 : 
      return 2 #hispanic or latino
   if row['H1GI6A'] == 1:
      return 3 #White
   if row['H1GI6B'] == 1:
      return 4 #Black or African American
   if row['H1GI6C'] == 1:
      return 5 #American Indian or Native American
   if row['H1GI6D'] == 1:
      return 6 #Asian or Pacific Islander
